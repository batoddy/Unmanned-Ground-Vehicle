...
#define ROLE_PIN 21
#define EMERGENCY_CRC 21
#define OK_CRC 12
#define LORA_OK 42
#define LORA_EMERGENCY 24
...
//............AYARLAR..............
const char *ayarlar2[] = {"AT+WMCFG=0",     // 0 coordinatör, 1 Node, 4 default
...
void setup()
{
  ...
  yeni yarmode fonksiyonunda gerçektende parametre ayarları verilmiş kontrol etmeniz gerekli
  aksi taktirde ağa bağlanmama sorunları yaşanacaktır.
  */
...
void loop()
{
  ...
  /*if (millis() > kanalBekleme_sure + kanalBekleme_bekleme)
  {
    ...
    e++;
    strcpy(mesaj.type, "Fixaj Merkez");
    *((int *)mesaj.temp) = e;
    gonder(&mesaj);
  ...
  while (fixajSerial.available())
  {
    receive(&mesaj);
    Serial.print("type: ");
    Serial.println(mesaj.type[0]);
    Serial.print("crc: ");
    Serial.println(mesaj.crc);
    ...
    //if ((mesaj.type == "LORA_OK" && mesaj.crc == OK_CRC) || (mesaj.type == "LORA_EMERGENCY" && mesaj.crc == EMERGENCY_CRC))
    //{
      switch (mesaj.type[0])
      {
      case LORA_OK:
        Serial.println("LORA_OK get - sending");
        gonder(&mesaj);
        emergency_flag = 0;
        break;
