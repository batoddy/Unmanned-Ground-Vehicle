...
volatile uint32_t command = LORA_OK;
...
void setup()
{
  ...
  attachInterrupt(digitalPinToInterrupt(BTN_PIN), btn_press_callback, CHANGE);
...
void loop()
{
  ...
  if (millis() > kanalBekleme_sure + kanalBekleme_bekleme)
  {
    ...
    tx_counter++;
    mesaj.data = command;
    mesaj.count = tx_counter;
    mesaj.crc = 12;

    gonder(&mesaj);

    Serial.println("Code: LORA_OK sent.");
    Serial.print("Packet: LORA_OK ");
