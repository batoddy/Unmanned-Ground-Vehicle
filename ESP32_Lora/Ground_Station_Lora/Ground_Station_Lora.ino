...
void btn_pressed_callback()
{
  Serial.println("rising_cb");
  emergency_flag = 1;
}

void btn_depressed_callback()
{
  Serial.println("falling_cb");
  emergency_flag = 0;
}
...
void setup()
{
  ...
  */
  ...
  attachInterrupt(digitalPinToInterrupt(BTN_PIN), btn_pressed_callback, RISING);
  attachInterrupt(digitalPinToInterrupt(BTN_PIN), btn_depressed_callback, FALLING);
}
...
void loop()
{
  ...
  if (millis() > kanalBekleme_sure + kanalBekleme_bekleme)
  {
    ...
    if (emergency_flag == 0)
      ...
      command = LORA_EMERGENCY;

    mesaj.type[0] = command;
    mesaj.crc = (command == LORA_OK) ? OK_CRC : EMERGENCY_CRC;
    gonder(&mesaj);
